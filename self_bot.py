...
==========================================================
همه‌چیز از طریق کال‌بک‌ها و پیام‌هایی که Worker کلادفلر فوروارد
می‌کنه پردازش می‌شه. کیف‌پول/کاربرها/سفارش‌ها/کانال‌های اجباری/
قیمت پنل‌ها همه تو Worker (Cloudflare KV) ذخیره می‌شن و این سرویس
از طریق /api/* با Worker صحبت می‌کنه. داده‌های خاص خودِ ریل‌وی
(سشن سلف، پروفایل، پست‌های لایکی، فلوهای مکالمه) در Upstash Redis.
...
# ============================================================
# Worker API helpers (کیف‌پول/کاربر/سفارش/کانال/قیمت واقعاً اینجاست)
# ============================================================
...
async def set_route_state(chat_id, active: bool):
    """به Worker می‌گه پیام‌های متنی بعدی این چت رو هم فوروارد کنه (وسط یه فلوی چندمرحله‌ای)."""
...
async def self_menu_kb(user_id):
    ...
    return {
        "inline_keyboard": [
            [{"text": "🕒 ساعت کنار اسم", "callback_data": "self:toggle_clock"}, {"text": "🔤 فونت اسم", "callback_data": "self:set_font_name"}],
            [{"text": "🔒 عضویت اجباری پیوی", "callback_data": "self:set_forced_pm"}, {"text": "💬 پیام خودکار", "callback_data": "self:set_autoreply"}],
            [{"text": "🎯 چالش لایکی", "callback_data": "self:likey_menu"}, {"text": "💳 وضعیت", "callback_data": "self:status"}],
...
async def finish_login(chat_id, user_id, client):
    ...
    await bot_send(chat_id, "✅ سلف با موفقیت فعال شد و اکانتت متصله! از همین منو بقیه‌ی تنظیمات رو انجام بده.",
...
async def handle_self_callback(cq):
    ...
    if data == "self:activate":
        ...
        await bot_answer_cb(cq["id"], "✅ فعال‌سازی انجام شد.")
    ...
    if data == "self:logout":
        ...
        await bot_send(chat_id,
            f"🚪 با موفقیت از اکانتت خارج شدی.\n\n⚠️ برای فعال‌سازی مجدد سلف، باید دوباره {SELF_ACTIVATION_COST:,} تومان پرداخت کنی.",
            await self_menu_kb(user_id))
        return
...
async def clock_loop(user_id: int):
    while user_id in active_clients:
        ...
        if client and prof.get("clock_on"):
            ...
            try:
                await client(UpdateProfileRequest(first_name=f"{base} {now.strftime('%H:%M')}".strip()))
            except Exception:
                pass
...
async def hourly_billing_loop():
    """با next_bill_at دقیق برای هر کاربر (نه یه sleep(3600) سراسری) تا درایفت نداشته باشه."""
    while True:
...
async def referral_reversal_loop():
    """هر ۲۰ دقیقه چک می‌کنه زیرمجموعه‌ها هنوز عضو کانال‌های اجباری هستن یا نه."""
    while True:
...
# ============================================================
# لایکی (کلیک عمومی روی دکمه‌ی پست‌های ساخته‌شده توسط کاربرهای سلف)
# ============================================================
    ...
    if data == "admin:orders":
        ...
        for o in orders[:15]:
            await bot_send(chat_id,
                ...
                {"inline_keyboard": [[{"text": "✅ تحویل بده", "callback_data": f"admin:deliver:{o['id']}"}]]})
        return
