...
UPSTASH_URL = os.environ["UPSTASH_URL"]        # فقط برای داده‌های خودِ ریل‌وی (سلف/لایکی/فلوها)
UPSTASH_TOKEN = os.environ["UPSTASH_TOKEN"]
RAILWAY_SECRET = os.environ.get("RAILWAY_SECRET", "")
WORKER_URL = os.environ["WORKER_URL"]          # آدرس Worker کلادفلر - برای خوندن/نوشتن کیف‌پول (KV)
...
# ============================================================
# کیف‌پول - از طریق API داخلی Worker (چون داده‌ی واقعی تو KV کلادفلره)
# ============================================================
async def get_wallet(user_id) -> int:
    async with httpx.AsyncClient() as c:
        r = await c.post(
            f"{WORKER_URL}/api/wallet/get",
            json={"user_id": user_id},
            headers={"X-Secret": RAILWAY_SECRET},
        )
        data = r.json()
...
async def get_all_users():
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{WORKER_URL}/api/users", headers={"X-Secret": RAILWAY_SECRET})
        data = r.json()
